function editar(id) {
  ...
  $.get(
    "modulos/administracion/entidades/formulario.php?id=" + id,
    function (dato) {
...
function listado() {
  ...
  $.get("modulos/administracion/entidades/listado.php", function (dato) {
    ...
    $("#listado").fadeIn("slow", function () {
      ...
      if ($.fn.DataTable.isDataTable("#dataTable")) {
        ...
        console.log(
          "Instancia de DataTable existente para Entidades destruida."
      ...
      console.log("DataTable inicializada para Entidades."); // Mensaje para depuración
    }); // Fin del callback de fadeIn
  }).fail(function (xhr, status, error) {
    console.error(
      "AJAX Error loading listado de Entidades: " + status,
      error,
      xhr
    );
    $("#mensaje").css("display", "block");
    $("#mensaje").html(
      '<div class="alert alert-danger" role="alert"><button type="button" class="close" data-dismiss="alert">&times;</button><i class="fas fa-exclamation-triangle"></i> Error al cargar el listado de Entidades: ' +
        status +
        "</div>"
    );
    $("#mensaje").fadeIn("slow");
    ...
    buttons: {
      confirm: {
        ...
        action: function () {
          ...
          $.ajax({
            type: "POST",
            url: "modulos/administracion/entidades/controlador.php?f=editar",
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
  ...
  $.confirm({
    ...
    buttons: {
      confirm: {
        ...
        action: function () {
          $.post(
            "modulos/administracion/entidades/controlador.php?f=eliminar",
            { id: id },
            function (dato) {
              $("#mensaje").css("display", "block");
              $("#mensaje").html(dato);
...
function ver_datos(id) {
  $.get(
    "modulos/administracion/entidades/ver_datos.php",
    {
      id: id,
    },
    function (data) {
