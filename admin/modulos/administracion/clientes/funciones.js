// docedes-plantillas/plantilla/modulos/administracion/clientes/funciones.js
...
function editar(id) {
  ...
  // Cargar el formulario de edición/creación del cliente
  $.get(
    "modulos/administracion/clientes/formulario.php?id=" + id,
    function (dato) {
...
function listado() {
  ...
  $.get("modulos/administracion/clientes/listado.php", function (dato) {
    ...
    $("#listado").fadeIn("slow", function () {
      ...
      if ($.fn.DataTable.isDataTable("#dataTable")) {
        ...
        console.log("Instancia de DataTable existente destruida."); // Mensaje para depuración
      }
      ...
      console.log("DataTable inicializada."); // Mensaje para depuración
    }); // Fin del callback de fadeIn
  }).fail(function (xhr, status, error) {
    //
    console.error("AJAX Error loading listado: " + status, error, xhr); //
      ...
      confirm: {
        ...
        action: function () {
          ...
          $.ajax({
            type: "POST",
            url:
              "modulos/administracion/clientes/controlador.php?f=editar&id=" +
              id,
            data: formData,
            cache: false,
            contentType: false,
...
function eliminar(id) {
  $.confirm({
    ...
    content: "Desea eliminar este cliente? Esta acción no se puede deshacer.",
    icon: "far fa-question-circle",
    animation: "scale",
    closeAnimation: "scale",
    opacity: 0.5,
    buttons: {
      confirm: {
        ...
        action: function () {
          $.post(
            "modulos/administracion/clientes/controlador.php?f=eliminar",
            { id: id },
            function (dato) {
              $("#mensaje").css("display", "block");
              $("#mensaje").html(dato);
...
function ver_datos(id) {
  $.get(
    "modulos/administracion/clientes/ver_datos.php",
    {
      id: id,
    },
    function (data) {
